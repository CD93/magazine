...
Utils.getSize = function(elements, events) {
	...
	var $styles = $('<style>tlt{z-index:9999;position:absolute;top:0.5em;left:0.5em;display:block;padding:0.5em 0.75em;font:1em/1 sans-serif;color:#fff;background:#00f;border-radius:3px;}</style>');
	var array = [];
...
Utils.smoothScroll = function() {
	...
	if (location.pathname.replace(/^\//, '') == this.pathname.replace(/^\//, '') && location.hostname == this.hostname) {
		...
		if ($target.length) {
			TweenLite.to(window, 0.8, { scrollTo: { y: $target.offset().top }, ease: Expo.easeInOut });
			return false;
		}
	}
};
